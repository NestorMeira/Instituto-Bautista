...
const Cuartovista = () => {
  ...
  const fetchData = async () => {
    const querySnapshot = await getDocs(query(collection(db, 'novedades'), where("categoria", "==", "Cuarto")));
...
{showPasswordForm ? (
      ...
      {novedades.map(novedad => (
        ...
        {novedad.imagen && <img src={novedad.imagen} alt="imagen" />}
          ...
          {novedad.link && <a href={novedad.link} target="_blank" rel="noopener noreferrer">- ABRIR LINK -</a>}
           </div>
      ))}
