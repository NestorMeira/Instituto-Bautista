...
const Sala5vista = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      let authorized = false; // Para verificar si la categoría es correcta
      querySnapshot.forEach((doc) => {
        const userData = doc.data();
        if (userData.nombre === nombre && userData.clave === clave) {
          // Si encuentra alguna coincidencia de nombre y contraseña, autentica al usuario
          authenticated = true;
          if (userData.categoria === "Sala de 5") {
      ...
      if (authenticated && authorized) {
        // Si se autentica y está autorizado, muestra un mensaje de éxito
        ...
        // Si no encuentra ninguna coincidencia o no está autorizado, muestra un mensaje de error
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'Nombre de usuario, contraseña o categoría incorrectos. Inténtalo de nuevo.',
        });
      }
    } catch (error) {
  ...
  const fetchData = async () => {
    const querySnapshot = await getDocs(query(collection(db, 'novedades'), where("categoria", "==", "Sala 5")));
